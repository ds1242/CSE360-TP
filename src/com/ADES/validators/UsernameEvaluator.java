...
public class UsernameEvaluator {
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 1: 
				...
				// . -> State 2
					...
					nextState = 3;
					userNameSize++; //count the new char.
				}
				// If it is none of those characters, the FSM halts
				else
					running = false;
			...
			case 3:
				// State 3 deals with a character after an ampersand in the name.
				// A-Z, a-z, 0-9 -> State 1
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
						(currentChar >= 'a' && currentChar <= 'z' ) ||	// Check for a-z
						(currentChar >= '0' && currentChar <= '9' )) {	// Check for 0-9
			...
			if (userNameSize > MAX_USERNAME_SIZE)
				running = false;
		...
		case 0:
			...
			userNameRecognizerErrorMessage += "A UserName must start with\nA-Z or a-z.\n";
		...
		case 1:
			...
			else if (userNameSize > MAX_USERNAME_SIZE) {
				// UserName is too long
				userNameRecognizerErrorMessage += 
					"A UserName must have no more than " + MAX_USERNAME_SIZE + " characters.\n";
				return userNameRecognizerErrorMessage;
			}
		...
		case 3:
			...
			userNameRecognizerErrorMessage +=
				"A UserName character after an ampersand\nmust be A-Z, a-z, 0-9.\n";
			return userNameRecognizerErrorMessage;
		default:
